...
static void quick_home_xy() {
  ...
  sensorless_t stealth_states{
      NUM_AXIS_LIST(TERN0(X_SENSORLESS, tmc_enable_stallguard(stepperX)),
                    TERN0(Y_SENSORLESS, tmc_enable_stallguard(stepperY)), false,
...
// activation, depending on the side of homing
  ...
  const bool pe_final_change_must_unpark = parking_extruder_unpark_after_homing(
...
#define _UNSAFE(A) TERN0(Z_SAFE_HOMING, homeZZ &&axis_should_home(_AXIS(A)))
  ...
  const bool homeZZ = TERN0(HAS_Z_AXIS, parser.seen_test('Z')),
             NUM_AXIS_LIST_( // Other axes should be homed before Z safe-homing
                 needX = _UNSAFE(X), needY = _UNSAFE(Y),
                 needZ = false, // UNUSED
  ...
  const bool may_skate =
      seenR NUM_AXIS_GANG(|| doX, || doY, || TERN0(Z_SAFE_HOMING, doZ), || doI,
                          || doJ, || doK, || doU, || doV, || doW);
  ...
  if (seenR && z_homing_height == 0) {
    ...
    // The "height" for Z is a coordinate. But if Z is not trusted/homed make it
    // relative.
...
// Restore the active tool after homing
  ...
  if (ENABLED(NANODLP_Z_SYNC) &&
      (ENABLED(NANODLP_ALL_AXIS) || TERN0(HAS_Z_AXIS, doZ)))
